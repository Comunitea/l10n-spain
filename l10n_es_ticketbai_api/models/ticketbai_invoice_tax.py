...
class TicketBaiTax(models.Model):
    ...
    not_subject_to_cause = fields.Selection(selection=[
        (NotSubjectToCause.OT.value, 'OT'),
        (NotSubjectToCause.RL.value, 'RL'),
    ], string='Not Subject to Cause', help="""
    OT:
    ...
    exempted_cause = fields.Selection(selection=[
    ...
    """)
    not_exempted_type = fields.Selection(selection=[
        (NotExemptedType.S1.value, 'S1'),
        (NotExemptedType.S2.value, 'S2')
    ], string='Not Exempted Type', help="""
    ...
    re_amount_total = fields.Char(
        'Surcharge Amount Total', default='',
        help='String of float with 12 digits and 2 decimal points.')
    surcharge_or_simplified_regime = fields.Selection(selection=[
        ...
        for record in self:
            if record.is_subject_to:
                tbai_utils.check_str_decimal(_(
                    "TicketBAI Invoice %s: Tax Base"
                ) % record.tbai_invoice_id.name, record.base)
        ...
        for record in self:
            if not record.is_subject_to and not record.not_subject_to_cause:
                raise exceptions.ValidationError(_(
                    "TicketBAI Invoice %s: Tax Not subject to cause is required."
    ...
    def _check_is_exempted(self):
        for record in self:
            if record.is_exempted and not record.is_subject_to:
                raise exceptions.ValidationError(_(
                    "TicketBAI Invoice %s: Exempted taxes are Subject to taxes."
                ) % record.tbai_invoice_id.name)
    ...
    def _check_exempted_cause(self):
        for record in self:
            if record.is_subject_to and record.is_exempted and \
                    not record.exempted_cause:
                raise exceptions.ValidationError(_(
                    "TicketBAI Invoice %s: Tax Exempted cause is required."
                ) % record.tbai_invoice_id.name)
    ...
    def _check_not_exempted_type(self):
        for record in self:
            if record.is_subject_to and not record.is_exempted and \
                    not record.not_exempted_type:
                raise exceptions.ValidationError(_(
                    "TicketBAI Invoice %s: Tax Not exempted type is required."
                ) % record.tbai_invoice_id.name)
    ...
    def _check_amount(self):
        for record in self:
            if record.is_subject_to and not record.is_exempted and record.amount:
                tbai_utils.check_str_percentage(_(
                    "TicketBAI Invoice %s: Tax Amount (%%)"
                ) % record.tbai_invoice_id.name, record.amount)
    ...
    def _check_amount_total(self):
        for record in self:
            if not record.is_subject_to or (
                ...
                tbai_utils.check_str_decimal(_(
                    "TicketBAI Invoice %s: Tax Amount Total"
                ) % record.tbai_invoice_id.name, record.amount_total)
    ...
    def _check_re_amount(self):
        for record in self:
            if record.is_subject_to and not record.is_exempted and \
                    ...
                    SurchargeOrSimplifiedRegimeType.S.value and record.re_amount:
                tbai_utils.check_str_percentage(_(
                    "TicketBAI Invoice %s: Tax Surcharge Amount (%%)"
                ) % record.tbai_invoice_id.name, record.re_amount)
    ...
    def _check_re_amount_total(self):
        for record in self:
            if record.is_subject_to and not record.is_exempted and \
                    record.surcharge_or_simplified_regime == \
                    SurchargeOrSimplifiedRegimeType.S.value and record.re_amount_total:
                tbai_utils.check_str_decimal(_(
    ...
    def _check_surcharge_or_simplified_regime(self):
        for record in self:
            if record.is_subject_to and not record.is_exempted and \
                    record.tbai_invoice_id.company_id.tbai_vat_regime_simplified and (
