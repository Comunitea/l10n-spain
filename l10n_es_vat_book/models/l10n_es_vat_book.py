...
class L10nEsVatBook(models.Model):
    ...
    def upsert_book_line_tax(self, move_line, vat_book_line, implied_taxes):
        ...
        if vat_book_line['special_tax_group']:
            base_line = filter(lambda l: not l['special_tax_group'],
                               implied_lines)
            base_line = (base_line and base_line[0]) or {}
            special_line = filter(lambda l: l['special_tax_group'],
                                  implied_lines)
            special_line = (special_line and special_line[0]) or {}
            if base_line and special_line:
                base_line.update({
                    'special_tax_id': special_line.get('tax_id', 0),
                    'special_tax_amount': special_line.get('tax_amount', 0),
                    'total_amount_special_include':
                        base_line.get('total_amount', 0) + special_line.get(
                            'tax_amount', 0),
                })
    ...
    def button_confirm(self):
        ...
        return super(L10nEsVatBook, self).button_confirm()

    def export_xlsx(self):
        self.ensure_one()
        context = dict(self.env.context, active_ids=self.ids)
        name = self.get_report_file_name()
        return {
            'name': name,
            ...
            'report_type': 'xlsx',
            'report_file': 'l10n_es_vat_book.l10n_es_vat_book',
            'context': context,
        }
